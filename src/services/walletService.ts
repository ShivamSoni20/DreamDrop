...
export async function signClaimAuthorization(payload:ClaimChallenge){
  if (appConfig.dataMode === "mock") return `0xmocksignature${payload.id.replace(/[^a-z0-9]/gi,"")}`;
  const provider = window.ethereum;
  if (!provider) throw new Error("No injected wallet found.");
  const accounts = await provider.request({ method: "eth_accounts" }) as string[];
  const account = accounts[0];
  if (!account || account.toLowerCase() !== payload.walletAddress.toLowerCase()) throw new Error("Connect the wallet this claim was prepared for.");
  return provider.request({ method: "personal_sign", params: [payload.message, account] }) as Promise<string>;
}
