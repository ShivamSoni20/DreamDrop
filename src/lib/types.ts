...
export interface Market {
  ...
  bestUpCashout: number | null;
  bestDownCashout: number | null;
  status: MarketStatus;
  liquidity: Liquidity;
  expiresAt: number;
...
export interface Campaign {
  ...
  totalDrops: number;
  claimedDrops: number;
  upDistributed: number;
  downDistributed: number;
  cashOuts: number;
...
export interface PreparedCampaign extends CreateCampaignInput {
  completeSets: number;
  upDrops: number;
  downDrops: number;
  totalDrops: number;
...
export interface CashoutQuote {
  ...
  positionId: string;
  marketProbability: number;
  bestExecutablePrice: number | null;
  quantityRequested: number;
  executableQuantity: number;
  estimatedProceeds: number | null;
  minimumProceeds: number | null;
  expiresAt: number;
  canCashOut: boolean;
}
